...
function collectOrderData() {
    ...
    $('#orderItemsTable tbody tr').each(function() {
        ...
        items.push({
            ...
            tax_rate: parseFloat($(this).find('.tax-rate').val()),
            discount_rate: parseFloat($(this).find('.discount-rate').val())
        });
    });
...
function createOrder(isDraft = false) {
    try {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                throw new Error(xhr.responseJSON?.error || 'Failed to create order');
            }
        });
    } catch (error) {
        Swal.fire({
